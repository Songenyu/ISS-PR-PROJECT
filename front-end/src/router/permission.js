...
export default (store, routes, to, next) => {
  let roleId = store.getters['user/getroleId']
  console.log('permission(store,....)', roleId)
  if (roleId !== 0 && !roleId) {
    next(new Error('roleId字段不能为空'))
    return false
  }
  let protectedRoutes = store.getters['permission/getPermission'](roleId)
